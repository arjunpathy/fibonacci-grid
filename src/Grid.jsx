...
const Grid = () => {
  ...
  const [enableCell, setCellState] = useState(true);
  ...
  const handleClick = (row, col) => {
    if (enableCell) {
      ...
      checkAndClearFibonacci(newGrid, cellsToHighlight, cellsToHighlight.length);
      setTimeout(() => {setHighlightedCells([]);}, 500);
    }
  };

  const checkAndClearFibonacci = (newGrid, highlightedCellsArray, length) => {
    let fibo = [];
    for (let k = 0; k < length; k++) {
      const [i, j] = highlightedCellsArray[k];
  ...
  const isFibonacci = (arr) => {
    return arr.every((ele, index) => ele === FIBONACCI_SEQUENCE[index]);
  }; // returns true if the arr is a fibonacci sequence
    ...
    >
      {grid.map((row, rowIndex) =>
        row.map((value, colIndex) => {
          ...
          const isFibonacci = fibonacciCells.some(
            ([r, c]) => r === rowIndex && c === colIndex
          );
